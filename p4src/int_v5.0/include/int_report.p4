...
control Int_report(inout headers hdr, inout metadata meta, in egress_intrinsic_metadata_t standard_metadata, in egress_intrinsic_metadata_from_parser_t imp) {
        ...
        action send_report(bit<48> dp_mac, bit<32> dp_ip, bit<48> collector_mac, bit<32> collector_ip, bit<16> collector_port) {
            ...
            if (hdr.nested_ipv4.isValid()) {
                // gre + ip
                // hdr.report_ipv4.totalLen = hdr.report_ipv4.totalLen + ((bit<16>)(8 + 20));
                hdr.ipv4.srcAddr = hdr.nested_ipv4.srcAddr;
                hdr.ipv4.dstAddr = hdr.nested_ipv4.dstAddr;
                hdr.ipv4.totalLen = hdr.ipv4.totalLen - ((bit<16>)(8 + 20)); 
                hdr.ipv4.protocol = hdr.nested_ipv4.protocol;
            }
            hdr.gre.setInvalid();
            hdr.nested_ipv4.setInvalid();
            ...
            table tb_int_reporting {
                ...
                // size = 512;
                key = {
                    hdr.ipv4.dstAddr: lpm; // TODO use default without key
                }
                actions = {
                    send_report;
                }
