...
/*        meta.int_metadata.instance_type = 0;*/
    ...
    state parse_gre {
        ...
        // TODO check nested proto
        transition parse_nested_ipv4;
    }
    state parse_nested_ipv4 {
        packet.extract(hdr.nested_ipv4);
        // overwrite with pod addresses
        meta.layer34_metadata.ip_src = hdr.nested_ipv4.srcAddr;
        meta.layer34_metadata.ip_dst = hdr.nested_ipv4.dstAddr;
        transition select(hdr.nested_ipv4.protocol) {
            8w0x11: parse_udp;
            8w0x6: parse_tcp;
            default: accept;
        }
    ...
    state parse_int_data_32 {
        packet.extract(hdr.int_data, 32);
        transition accept;
    }

    state parse_int_data_64 {
        packet.extract(hdr.int_data, 64);
        transition accept;
    }

    state parse_int_data_96 {
        packet.extract(hdr.int_data, 96);
        transition accept;
    }

    state parse_int_data_128 {
        packet.extract(hdr.int_data, 128);
        transition accept;
    }

    state parse_int_data_160 {
        packet.extract(hdr.int_data, 160);
        transition accept;
    }

    state parse_int_data_192 {
        packet.extract(hdr.int_data, 192);
        transition accept;
    }

    state parse_int_data_224 {
        packet.extract(hdr.int_data, 224);
        transition accept;
    }

    state parse_int_data_256 {
        packet.extract(hdr.int_data, 256);
        transition accept;
    }

    state parse_int_data_288 {
        packet.extract(hdr.int_data, 288);
        transition accept;
    }

    state parse_int_data_320 {
        packet.extract(hdr.int_data, 320);
        transition accept;
    }

    state parse_int_data_384 {
        packet.extract(hdr.int_data, 384);
        transition accept;
    }

    state parse_int_data_448 {
        packet.extract(hdr.int_data, 448);
        transition accept;
    }

    state parse_int_data_480 {
        packet.extract(hdr.int_data, 480);
        transition accept;
    }

    state parse_int_data_512 {
        packet.extract(hdr.int_data, 512);
        transition accept;
    }

    state parse_int_data_576 {
        packet.extract(hdr.int_data, 576);
        transition accept;
    }

    state parse_int_data_640 {
        packet.extract(hdr.int_data, 640);
        transition accept;
    }

    state parse_int_data_672 {
        packet.extract(hdr.int_data, 672);
        transition accept;
    }

    state parse_int_data_768 {
        packet.extract(hdr.int_data, 768);
        transition accept;
    }

    state parse_int_data_864 {
        packet.extract(hdr.int_data, 864);
        transition accept;
    }

    state parse_int_data_896 {
        packet.extract(hdr.int_data, 896);
        transition accept;
    }

    state parse_int_data_960 {
        packet.extract(hdr.int_data, 960);
        transition accept;
    }

    state parse_int_data_1024 {
        packet.extract(hdr.int_data, 1024);
        transition accept;
    }

    state parse_int_data_1152 {
        packet.extract(hdr.int_data, 1152);
        transition accept;
    }

    state parse_int_data_1280 {
        packet.extract(hdr.int_data, 1280);
        transition accept;
    }
}
...
control computeChecksum(inout headers hdr, inout metadata meta) {
    apply {
        ...
        // update_checksum(
        //     hdr.nested_ipv4.isValid(),
        //     {
        //         hdr.nested_ipv4.version,
        //         hdr.nested_ipv4.ihl,
        //         hdr.nested_ipv4.dscp,
        //         hdr.nested_ipv4.ecn,
        //         hdr.nested_ipv4.totalLen,
        //         hdr.nested_ipv4.id,
        //         hdr.nested_ipv4.flags,
        //         hdr.nested_ipv4.fragOffset,
        //         hdr.nested_ipv4.ttl,
        //         hdr.nested_ipv4.protocol,
        //         hdr.nested_ipv4.srcAddr,
        //         hdr.nested_ipv4.dstAddr
        //     },
        //     hdr.nested_ipv4.hdrChecksum,
        //     HashAlgorithm.csum16
        // );
        ...
        update_checksum_with_payload(
            hdr.udp.isValid() && hdr.nested_ipv4.isValid(), 
            {  hdr.nested_ipv4.srcAddr, 
                hdr.nested_ipv4.dstAddr, 
                8w0, 
                hdr.nested_ipv4.protocol, 
