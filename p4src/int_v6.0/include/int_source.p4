...
control Int_source(inout headers hdr, inout metadata meta, in ingress_intrinsic_metadata_t standard_metadata, in ingress_intrinsic_metadata_from_parser_t imp) {
    ...
    // collection_id - custom identifier assigned by control plane to distinguish different telemetry requests
    action configure_source(bit<8> max_hop, bit<5> hop_metadata_len, bit<5> ins_cnt, bit<16> ins_mask, bit<7> collection_id) {
        ...
        hdr.int_header.c = 0;
        hdr.int_header.e = 0;
        hdr.int_header.collection_id = collection_id;
        ...
        // // X
        // if(hdr.nested_ipv4.isValid()) {
        //     hdr.nested_ipv4.totalLen = hdr.nested_ipv4.totalLen + INT_ALL_HEADER_LEN_BYTES;  // adding size of INT headers
        // }
        // // X
    ...
    table tb_int_source_tunneled {
        actions = {
            configure_source;
        }
        key = {
            hdr.nested_ipv4.srcAddr     : ternary;
            hdr.nested_ipv4.dstAddr     : ternary;
            meta.layer34_metadata.l4_src: ternary;
            meta.layer34_metadata.l4_dst: ternary;
        }
        size = 127;
    ...
    apply {
        ...
        if (meta.int_metadata.source == 1) {  
            ...
            if (hdr.nested_ipv4.isValid()) {
                if (!tb_int_source_tunneled.apply().hit) {
                    tb_int_source.apply();
                }
            } else {
                tb_int_source.apply();
